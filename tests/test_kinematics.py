...
class TestOptimalTrajectory(unittest.TestCase):
    def test_standing_still_when_nothing_to_do(self):
        start = end = State(1.234, 0.)
        profiles = optimal_trajectory(xEnd=end.position, state=start)
        final = final_state(start, profiles)

        self.assertEqual(profiles, [])
        self.assertEqual(final, end)
    ...
    def test_critical_profiles(self):
        ...
        profiles = optimal_trajectory(xEnd=end.position, vEnd=end.velocity, state=start, maxAcc=maxAcc)

        self.assertEqual(profiles, [(1.0, maxAcc)])
        self.assertEqual(final_state(start, profiles), end)
    ...
    def test_right_place_but_not_the_right_speed(self):
        ...
        assert_allclose(final_state(start, profiles), end)
    ...
    def test_we_can_cruise_for_some_time(self):
        ...
        maxSpeed = 2.0
        maxAcc = 1.0
        profiles = optimal_trajectory(xEnd=end.position, vEnd=end.velocity, state=start, maxSpeed=maxSpeed, maxAcc=maxAcc)
